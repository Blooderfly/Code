...
class CrackPawn_ZedBloat_Versus_Flame extends KFPawn_ZedBloat;
var LinearColor AlphaKingClotGlowColor;
var export editinline ParticleSystemComponent BurningEffect;

simulated function PostBeginPlay()
{

    super(KFPawn_Monster).PostBeginPlay();
    AfflictionHandler.FireFullyCharredDuration = 2.0;
    AfflictionHandler.FireCharPercentThreshhold = 0.20;
...
simulated function UpdateGameplayMICParams()
{
    ...
    if(MIC != none)
    {
        MIC.SetVectorParameterValue('Vector_FresnelGlowColor', AlphaKingClotGlowColor);
        MIC.SetVectorParameterValue('Vector_GlowColor', AlphaKingClotGlowColor);
        MIC.SetScalarParameterValue('MetalFresnelGlow', 0.70);
        MIC.SetScalarParameterValue('FresnelAll', 0.70);
        MIC.SetScalarParameterValue('Scalar_Burnt', 0.70);
    }
    if(WorldInfo.NetMode != NM_DedicatedServer)
    {
    	BurningEffect = WorldInfo.MyEmitterPool.SpawnEmitterMeshAttachment(particlesystem'FX_CHR_Fire', Mesh, 'hips', true);
    	BurningEffect.SetAbsolute(false, true, true);
    } 
}
...
defaultproperties
{
	AlphaKingClotGlowColor=(R=1.0,G=0.27,B=0.0,A=1.0)
    IntendedBodyScale=1.25 //1.5
	bVersusZed=false
    ...
    // Faster sprint
	SprintSpeed=600//445.0f //400 //420
	SprintStrafeSpeed=400.f//295.f //250
	Groundspeed=350//2.85f  //260 //250 //280
